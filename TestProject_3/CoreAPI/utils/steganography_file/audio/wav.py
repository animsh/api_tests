...
def channel_bytes(data, channel):
    ...
    assert channel_count(
        data) > channel, 'This data do not have the given channel'
...
def write_wav_file(storage_id, location, data, rate):
    # location = "/" + location
    print("write_wav_file")
    ...
    try:
        ...
        StorageEncode.objects.filter(id=storage_id).update(encoded_file=location)
    except PermissionError:
...
def show_information(file_data, rate):
    ...
    print(
        f"Number of total bytes that can be hidden: {bytes_to_hide_count(file_data)}")
...
def convert_audio_file(input_file, output_file, delete=False):
    ...
    try:
        subprocess.call(['ffmpeg', '-i', input_file, '-y',
                        output_file], stderr=subprocess.DEVNULL)
    except FileNotFoundError:
        return False
