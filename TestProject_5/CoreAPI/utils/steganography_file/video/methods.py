...
def hide(user_id, storage_id, video_file_input, video_file_output, message_file, will_shuffle=False, dict_index=None):
    ...
    writer = cv2.VideoWriter(video_file_output, cv2.VideoWriter_fourcc(
        *VIDEO_CODEC), fps, (width, height), True)
    ...
    if message_bytes_length > video_bytes_to_hide:
        ...
        raise Exception('Insufficient space to hide the message')
        # sys.exit()
    ...
    while video_file.isOpened():
        ...
        if start <= message_bytes_length:
            ...
            modified_frame = bm.hide_in_frame(
                frame, bytes_to_hide_sub_list, index_dict)
    ...
    StorageEncode.objects.filter(
        id=storage_id).update(encoded_file=video_file_output)

    # generate the file to retrieve the message
    utils.generate_key_file(user_id, storage_id,
                            message_file, len(message_bytes), index_dict)
...
def retrieve(video_file_location, key_file, user_id, storage_id, encrypted_password, private_key):
    ...
    while video_file.isOpened():
        ...
        if not ret:
            ...
            raise Exception('Invalid video file and/or keys file')
            # sys.exit()
