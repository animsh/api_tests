...
def hide(user_id, storage_id, audio_file_input, audio_file_output, message_file, will_shuffle=False, dict_index=None):
    ...
    # if not, then it was given an incorrect extension of a file
        ...
        raise Exception('The output file can only be .wav or .flac format')
        # sys.exit()

    # begin the encoding
    sequence_hide(user_id, storage_id, audio_file_input, audio_file_output,
                  message_file, will_shuffle, dict_index)
...
def retrieve(audio_file_input, key_file, user_id, storage_id, encrypted_password, private_key):
    """Method responsible to manage the files, it will convert the audio formats if necessary before starting the
    ...
    sequence_retrieve(audio_file_input, key_file, user_id,
                      storage_id, encrypted_password, private_key)
...
def sequence_hide(user_id, storage_id, audio_file, result_audio_file, message_file, shuffle=False, dict_index=None):
    ...
    if max_data < len(message_bytes):
        ...
        raise Exception('Not enough space to hide the message')
        # sys.exit()
    ...
    for i in range(channel_count):
        ...
        modified_channel_bytes = bm.hide_bytes(
            channel_bytes, message_bytes_sub_array, index_dict)

        # replace the data channel
        original_song = wav.replace_data_channel(
            original_song, modified_channel_bytes, i)

    # create the file
    wav.write_wav_file(storage_id, result_audio_file, original_song, rate)
...
def sequence_retrieve(audio_file, key_file, user_id, storage_id, encrypted_password, private_key):
    ...
    for i in range(channel_count):
        ...
        extracted_bytes = bm.retrieve_bytes(
            channel_bytes, bytes_to_get, dictionary)
    ...
    result = message.write_file(os.path.join(
        settings.MEDIA_ROOT, f"upload/{user_id}/decoded_files/{result_file}"), extracted_bytes_array)
