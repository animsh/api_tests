...
import datetime
from ...models import StorageEncode
...
def generate_key_file(user_id, storage_id, file_name, message_length, indexes_dictionary=None):
    ...
    filepath = os.path.join(
        settings.MEDIA_ROOT, f'upload/{user_id}/key_files/{datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}')

    # if file_directory != '':
    #     final_name = file_directory + '/' + str(datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")) + '_keys'
    # else:
    #     final_name = str(datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")) + '_keys'
    ...
    with open(filepath, 'w') as fp:
        ...
        print("Key file generated",  filepath.title)
        StorageEncode.objects.filter(id=storage_id).update(key_file=filepath)
...
def read_key_file(location):
    ...
    try:
        ...
        if data['method'] == 'shuffle':
            ...
            data['indexes_dictionary'] = {
                int(k): v for k, v in data['indexes_dictionary'].items()}
    except FileNotFoundError:
        print("The key file doesn't exist or no read permissions")
        raise Exception("The key file doesn't exist or no read permissions")
        # sys.exit()
    except KeyError:
        print("Key file is invalid")
        raise Exception("Key file is invalid")
        # sys.exit()
