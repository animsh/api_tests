...
def main(args_dictionary):
    ...
    if 'input_file' not in args_dictionary:
        ...
        raise Exception('Missing input file destination!')
        # sys.exit()
    ...
    if args_dictionary['operation'] == 'encode':
        ...
        if 'message_file' not in args_dictionary:
            ...
            raise Exception('Missing message file destination!')
            # sys.exit()
        ...
        user_id = args_dictionary['user_id']
        storage_id = args_dictionary['storage_id']

        methods.sequence_hide(user_id, storage_id, input_file, output_file,
    ...
    # decode option
        ...
        if 'key_file' not in args_dictionary:
            ...
            raise Exception('Missing key file destination!')
            # sys.exit()
        ...
        user_id = args_dictionary['user_id']
        storage_id = args_dictionary['storage_id']
        encrypted_password = args_dictionary['encrypted_password']
        private_key = args_dictionary['private_key']
        ...
        methods.sequence_retrieve(
            input_file, key_file, user_id, storage_id, encrypted_password, private_key)
